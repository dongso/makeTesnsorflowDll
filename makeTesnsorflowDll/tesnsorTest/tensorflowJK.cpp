...
float main()
{
	...
	Mat imgMat = imread("D:/my_projects/git_local_server/makeTesnsorflowDll/makeTesnsorflowDll/x64/Release/S8_A_02NG_01_L190.bmp", IMREAD_COLOR);
...
float* runCnnFullC1(int nx, int ny, int width, int height, unsigned char* pImage, int scatter)
{
	// declare a session
	Session* session;
	Status status = NewSession(SessionOptions(), &session);
	...
	cameraImage.convertTo(cameraImage, CV_32FC1, 1 / 255.0);	// 8uint-3channel  ->  32float-4channel
	...
	Tensor inputSample(DT_FLOAT, TensorShape({ 1, height, width, 1 }));
	float *ptrTensor = inputSample.flat<float>().data();
	cv::Mat bufferImg(height, width, CV_32FC1, ptrTensor);
	cameraImage.convertTo(bufferImg, CV_32FC1);		//
...
float* runCnnBlockC4(int nx, int ny, int width, int height, unsigned char* pImage, int scatter)
{
	...
	//cv::Mat cameraImage(height, width, CV_8UC3, pImage);		// C# image pointer to C++ Mat
	cv::Mat cameraImage = imread("D:/my_projects/git_local_server/makeTesnsorflowDll/makeTesnsorflowDll/x64/Release/S8_A_02NG_01_L190.bmp", IMREAD_COLOR);	
	cv::cvtColor(cameraImage, cameraImage, CV_BGR2RGB);
